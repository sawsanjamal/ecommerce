...
const { linkContactAndItem, getContactPurchasedItems } = require("./contact");
...
app.post("/download-email", (req, res) => {
  ...
  sendDownloadLink(
    ...
    res.json({ message: "Check your email" }).catch(() => {
...
app.post("/create-checkout-session", async (req, res) => {
  ...
  if (item == null) {
    return res.status.status(400).json({ message: "Invalid Item" });
  }
